...
webix.i18n.locales["es-ES"].pivot = {
  apply: "Aplicar",
	  ...
	  operationNotDefined: "Operación no definida",
	  layoutIncorrect: "pivotLayout debe ser una instancia de tipo Array",
	  pivotMessage: "Configurar pivot...",
	  popupHeader: "Ajustes del Pivot",
	  radar: "Radar",
	  ...
	  total: "Total",
	  values: "Valores",
	  valuesNotDefined: "Capo Valores o Grupos no han sido establecidos",
	  windowTitle: "Configuración del Pivot",
	  windowMessage: "arrastre los campos aquí"
};
	...
	function () {
	  ...
	  _createClass(Data, [{
	    ...
	    value: function processColumns(data, columns, structure, header, item, name) {
	      ...
	      if (columns.length > 0) {
	        name = name && name.length ? name + this.divider : "";

	        for (var i in data) {
	          if (!header[i]) header[i] = {};
	          data[i] = this.processColumns(data[i], columns.slice(1), structure, header[i], item, name + i);
	        }
	      } else {
	        ...
	        for (var id in data) {
	          ...
	          for (var _i = 0; _i < values.length; _i++) {
	            for (var j = 0; j < values[_i].operation.length; j++) {
	              var vname = values[_i].operation[j] + this.divider + values[_i].name;
	              if (!isUndefined(name)) vname = name + this.divider + vname;
	              if (duplicate[vname]) continue;
	              duplicate[vname] = true;
